                    ...
                    >
                      <Button
                        ...
                        className={`relative px-2.5 sm:px-3 lg:px-4 py-2 text-sm font-medium rounded-lg transition-all ${
                          isActive
                            ? "bg-gradient-to-r from-metamask-500 to-metamask-600 text-orange-500 shadow-md"
                            : "text-gray-800 hover:text-metamask-600 hover:bg-metamask-50 hover:shadow-sm"
                        }`}
                      >
              ...
              <SheetContent side="right" className="w-[280px] sm:w-[320px] p-6 bg-gradient-to-br from-white/95 to-gray-50/95 backdrop-blur-xl border-l border-white/30 shadow-2xl">
                <SheetHeader className="mb-6">
                  <SheetTitle className="text-left">
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 rounded-xl bg-gradient-to-br from-metamask-500/10 to-metamask-600/10 shadow-sm border border-metamask-500/20">
                        <Image src="/logo.png" alt="ConsizeN" width={36} height={36} className="rounded-lg" />
                      </div>
                      ...
                      <Link key={item.name} href={item.href}>
                        <Button
                          ...
                          className={`w-full justify-start px-4 py-5 text-base rounded-xl transition-all ${
                            isActive
                              ? "bg-gradient-to-r from-metamask-500 to-metamask-600 text-orange-500 shadow-md"
                              : "text-gray-800 hover:text-metamask-600 hover:bg-metamask-50/70 hover:shadow-sm"
                          }`}
                        >
                          <item.icon className={`h-5 w-5 mr-3 ${isActive ? 'animate-pulse' : ''}`} />
                          <span className="font-medium">{item.name}</span>
                          {isActive && (
                            <div className="absolute left-0 top-1/2 transform -translate-y-1/2 w-1.5 h-10 bg-gradient-to-b from-metamask-500 to-metamask-600 rounded-r-full shadow-[0_0_10px_rgba(247,147,26,0.6)]" />
                          )}
                        </Button>
                      </Link>
                    );
                ...
                <div className="absolute bottom-8 left-6 right-6">
                  ...
                  <div className="text-center text-sm text-gray-600">
                    <span className="font-medium text-metamask-600">ConsizeN</span> &copy; {new Date().getFullYear()} <br /> Sustainable Spending Agent
                  </div>
                </div>
              </SheetContent>
            </Sheet>
